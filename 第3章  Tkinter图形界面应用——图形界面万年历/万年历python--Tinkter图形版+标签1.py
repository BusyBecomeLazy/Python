...
class Calendar:
    ...
    def drawHeader(self, master):
        ...
        Label(master, text='年').grid(row=0, column=col_idx);
        col_idx += 1
        # 设置年份可选菜单OptionMenu项，OptionMenu功能与combox相似
        omYear = OptionMenu(master, self.vYear, *tuple(range(2005, 2010)))
        ...
        Label(master, text='月').grid(row=0, column=col_idx);
        col_idx += 1
        # 设置月份可选菜单OptionMenu项
        ...
        # 创建日组件        
        self.vDay.set(now[2])
        Label(master, text='日').grid(row=0, column=col_idx);
        col_idx += 1
        # 设置日可选菜单OptionMenu项
