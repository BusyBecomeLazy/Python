...
unvisited.append('http://www.zut.edu.cn/index/xwdt/301.htm')
...
while unvisited:
    ...
    #m=re.findall(r'<a href=\"([0-9a-zA-Z\_\/\.\%\?\=\-\&]+)\">',content,re.I)
    #m=re.findall(r'<a.*?href="(.+)".*?>(.*?)</a>',content,re.I)
    
    #m=re.findall(r"<a.*?href=.*?<\/a>",content,re.I)
    m=re.findall(r'<a href=\"(.+?)\" target=\"_blank\">',content,re.I)
    for x in m:
    ...
    m=re.findall(r'<a href=\"(.+?)\" class=\"Next\">',content,re.I)
    for x in m:
